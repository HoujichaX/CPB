...
- **CPB コア** (圧縮パイプライン, RS 符号, コンテナ形式, GUI アプリ):
  Apache License 2.0 — 商用・非商用を問わず自由に利用可能。

- **学習済み辞書 & 辞書学習ツール**:
  商用ライセンスあり。 特定データに最適化されたエンタープライズ辞書パッケージについてはお問い合わせください。

コアは商用辞書なしで完全に動作します。 cpb_train.exe で独自辞書を無料で作成できます。
